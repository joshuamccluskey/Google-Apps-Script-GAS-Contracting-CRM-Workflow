...
function processRowsAssigned(action, emailTemplate=null) {
  ...
  rows
      ...
      .forEach(row => {
        ...
        Logger.log(`Row ${row.rowIndex+4}: ${assigned}`);
      });
...
function processRowsApproved(action, emailTemplate=null) {
  ...
  rows
      ...
      .forEach(row => {
        ...
        try { 
         ...
         if (response == ui.Button.YES) {
          MailApp.sendEmail({
            to: row[REQUISITIONER],
            cc: row[EMAIL_ADDRESS],
            bcc: 'joshua.mccluskey@noaa.gov, ben.carlson@noaa.gov, ogie.a.olanday@noaa.gov',
            subject: `Purchase Notification: ${row[ACTION]}`,
            htmlBody: emailBody,
          });} else {}
        ...
        Logger.log(`Row ${row.rowIndex+3}: ${approved}`);
      });
...
function processRowsAwarded(action, emailTemplate=null) {
  ...
  rows
      ...
      .forEach(row => {
        ...
        Logger.log(`Row ${row.rowIndex+2}: ${awarded}`);
      });
