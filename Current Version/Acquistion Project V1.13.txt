...
function emailer() {
  GmailApp.sendEmail('joshua.mccluskey@noaa.gov, ben.carlson@noaa.gov, joseph.hwang@noaa.gov, jesse.pillai@noaa.gov', 'NOTIFICATION: Req Submission', 'Hello,\n\nPlease review FY 21 Req Log for submission:\n\n' + ss.getUrl())
}
...
/**
 * Wrapper function of `processRowsInfo` for the 'Info' action.
 */
...
function processRowsAssigned(action, emailTemplate=null) {
  ...
  rows
      ...
      .forEach(row => {
        ...
        try { 
         ...
         if (response == ui.Button.YES) {
          MailApp.sendEmail({
            ...
            bcc: 'joshua.mccluskey@noaa.gov, ben.carlson@noaa.gov',
        ...
        Logger.log(`Row ${row.rowIndex+6}: ${assigned}`);
      });
...
function processRowsApproved(action, emailTemplate=null) {
  ...
  rows
      ...
      .forEach(row => {
        ...
        try { 
         let ui = SpreadsheetApp.getUi();
         let response = ui.alert('Hold Up!','Are you sure you want to send the email?', ui.ButtonSet.YES_NO);
        ...
        Logger.log(`Row ${row.rowIndex+5}: ${approved}`);
      });
...
function processRowsAwarded(action, emailTemplate=null) {
  ...
  rows
      ...
      .forEach(row => {
        ...
        Logger.log(`Row ${row.rowIndex+4}: ${awarded}`);
      });
