  ...
  rows
      ...
      .forEach(row => {
        ...
        try { 
         ...
         let response = ui.alert('Hold Up!','Are you sure you want to send the email? \n\n *Choose "No" for Libby\'s purchases* \n\n Email and Timestamp Guide: \n\n - *Don\'t forget to cut and paste the timestamp from the \"Status\" column to the proper \"Log\" column. \n\n - An error will occur if you try sending an email before you cut and paste the timestamp from the \"Status\" column to the proper \"Log\" column \n\n - "Yes" will send a notificaiton email to the requisitioner and end-user of the action, and add a timestamp for the Action in the \"Status"\ column. \n\n - "No" will not send an email, but it\'ll add a timestamp for the Action in the \"Status"\ column.', ui.ButtonSet.YES_NO);
